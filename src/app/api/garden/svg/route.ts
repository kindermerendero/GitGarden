...
function renderPlant(plant: PlantData, xOff: number, baseY: number): string {
  ...
  for (let i = 0; i < stemBlocks; i++) {
    ...
    r.push(`<rect x="${stemX+PX}" y="${y+1}" width="1" height="${PX-1}" fill="${shadowColor(stemColor)}" opacity="0.40"/>`);
  }
  ...
  sprite.forEach((row, ri) => row.forEach((cell, ci) => {
    if (!cell) return;
    r.push(`<rect x="${fx+ci*PX+1}" y="${flowerTopY+ri*PX+1}" width="${PX}" height="${PX}" fill="${cell>=2?shadowColor(cOuter):shadowColor(petalColor)}" opacity="0.32"/>`);
  }));
  ...
  sprite.forEach((row, ri) => row.forEach((cell, ci) => {
    if (cell < 2) return;
    r.push(`<rect x="${fx+ci*PX}" y="${flowerTopY+ri*PX}" width="2" height="2" fill="${highlightColor(cell===3?cInner:cOuter)}" opacity="0.75"/>`);
  }));
...
function buildSvg(plants: PlantData[], bg: string, glow: string): string {
  ...
  parts.push(
    `<text x="${totalW-8}" y="${svgH-6}" font-family="Georgia,'Times New Roman',serif" ` +
    `font-size="9" fill="white" opacity="0.18" text-anchor="end">GitGarden</text>`
  );
  parts.push(`</svg>`);
  return parts.join("");
}
